...
const Home = () => {
  ...
  return (
    <main className="flex min-h-screen flex-col p-6 bg-background">
      <p className="text-4xl self-start">Ahmet Efe Akyazı</p>
      <div className="flex flex-row">
        <Image className="p-6 rounded-full max-h-[200px] self-center" src={"/pfp.png"} width={200} height={200} unoptimized alt="thumbnail" />
        <p className="text-base self-start text-foreground mt-10">Merhaba, ben Ahmet Efe Akyazı. 16 yaşındayım. Gömülü sistemler, elektronik, mobil uygulamalar ilgi alanlarım arasındadır.
          ...
          <br /><br /><Link href={"https://github.com/efe3535"} className="text-xl text-foreground"><b className="text-2xl">GitHub:</b> efe3535</Link>
        </p>
      </div>
      <p className="text-4xl self-start mt-20">Blog</p>
      <ul className="w-2/3">
        {allPostsData.map(({ id, date, title, img }) => (
          <li key={id}>
            <Link href={`/posts/${id}`} className="mt-10 border-b-2 border-r-0 bg-secondary hover:bg-tertiary transition p-5 rounded-lg border-b-foreground w-auto flex flex-row" key={id}>
              ...
              <Image className="ml-auto rounded-md" src={img} width={200} height={100} unoptimized alt="thumbnail" />
            </Link>
          </li>
        ))
        }
