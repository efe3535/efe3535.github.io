  ...
  return (
    <Suspense fallback={<Loading />}>
      <main className="flex min-h-screen flex-col p-6 bg-background">
        <p className="text-4xl self-start underline underline-offset-8 max-md:self-center ">Ahmet Efe Akyazı</p>
        <div className="flex flex-row max-md:flex-col">
          ...
          <p className="text-base self-start text-foreground mt-10">Merhaba, ben Ahmet Efe Akyazı. 16 yaşındayım. Gömülü sistemler, elektronik, mobil uygulamalar ilgi alanlarım arasındadır.
            ...
            <br /><br /><Link href={"https://github.com/efe3535"} className="text-xl text-foreground"><b className="text-2xl">GitHub:</b> <span className="underline bg-foreground2 hover:animate-pulse">efe3535</span></Link>
          </p>
        </div>
        <p className="text-4xl self-start mt-20 underline underline-offset-8">Blog</p>
        <ul className="w-2/3 max-md:w-full">
          {allPostsData.map(({ id, date, title, img }) => (
            <li key={id}>
              <Link href={`/posts/${id}`} className="mt-10 border-b-2 border-r-0 bg-secondary hover:bg-tertiary transition p-5 rounded-lg border-b-foreground w-auto flex" key={id}>
                ...
                <Image className="ml-auto rounded-md transition-opacity max-md:hidden" src={img} width={200} height={100} alt="thumbnail" />
              </Link>
            </li>
          ))
          }
